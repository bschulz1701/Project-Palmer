...
#include "Particle.h"
#line 1 "c:/Users/schu3119/Documents/Palmer-Boron/src/Palmer-Boron.ino"
/*
 * Project Palmer-Boron
 * Description:
 * Author:
 * Date:
 */
...
// #include "lib/Adafruit_SGP30_Sensor/src/Adafruit_SGP30.h"
#include "dct.h"
...
#include "MCP3221.h"
#include "MCP23008.h"
// #include "lib/OneWire/src/OneWire.h"
#include <DS18.h>
// #include <OneWire.h>
...
void setup();
void loop();
void TransmitData(String Message);
#line 58 "c:/Users/schu3119/Documents/Palmer-Boron/src/Palmer-Boron.ino"
...
#define UPDATE_PERIOD 300000 //Number of milliseconds between primary sensor updates, Normal log rate = 5 minutes
// #define UPDATE_PERIOD 10000 //Number of milliseconds between primary sensor updates, Testing log rate = 10 seconds
...
#define ONE_WIRE_BUS D4 //Converted to Boron pin number 
...
void setup() {
  #if defined(SQUAWK)
    Cellular.setActiveSim(INTERNAL_SIM);  //Setup sim
...
void loop() {
  ...
  digitalWrite(D7, HIGH); //Blink LED on transmit 
  if(UpdateCount == 0) {
  ...
  String Packet = ""; //Define bulk data string 
  // LoRa.beginPacket();
  //Print data
  #if defined(SHERLOCK)
    ...
    Serial1.print(',');
  #endif
    Packet = Packet + String(serialNumber) + ",";
  ...
  for(int i = 0; i < 16; i++) {  //FIX! hardcode
      ...
      Serial1.print(',');
    #endif
      Packet = Packet + String(RemoteData[i]) + ",";
  ...
  for(int i = 0; i < 3; i++) {
      ...
      Serial1.print(',');
    #endif
      Packet = Packet + String(TempData[i]) + "," + String(MoistData[i]) + ",";
...
void TransmitData(String Message) 
{
  #if defined(SQUAWK)  //Onlyt send when enabled 
    ...
    Particle.publish("Data", Message, WITH_ACK); //Publish values to cloud
    delay(1000); 
  #endif
  // }
...
//  pinMode(11, OUTPUT);
  // Serial1.begin(38400);
  // Serial1.println("Begin Palmer Remote Test...");
  IO.begin();
  adc.Begin();
  adc.SetResolution(12); //Set for low resolution, high read rate
  IO.PinMode(ONE_WIRE_IO, INPUT);
...
void GetRemoteData(boolean Baseline)
{
  ...
  RemoteData[15] = GetVoltage(); //FIX! Fix voltage call function, use library instead
    ...
    Serial1.println("Error Flags:");
    for(int i = 0; i < NUM_FLAGS_REMOTE; i++) {
      Serial1.println(RemoteErrorFlags[i]);
    }
  #endif
...
void GetArrayData() //Get data from temp/soil moisture array
{
  ...
  IO.PinMode(S1, OUTPUT);
  IO.DigitalWrite(S0, LOW); //Force to default to state
  IO.DigitalWrite(S1, LOW);
  IO.PinMode(IO_EN, OUTPUT);
  boolean S0_State = false; //Initialize state variables
  boolean S1_State = false;
  for(int i = 0; i < NUM_DEPTH_NODES; i++) {
    //SET MUX
    //DEBUG!
    // IO.DigitalWrite(S0, LOW);
    // IO.DigitalWrite(S1, HIGH);
    if(i == 0) {
      S0_State = false;
      S1_State = false;
    }
    ...
    // boolean S0_State = bool(i & 0x01); //Pull off lowest bit
    // boolean S1_State = bool((i >> 1) & 0x01); //Pull off next lowest bit
    #if defined(WHITE_RABBIT_OBJECT) 
      Serial1.print("ArrayVal = ");
      Serial1.println(i);
    #endif
    ...
    delay(250); //Used to provide time for the DS18B20 to initialize after the bus is pulled up //DEBUG!
    boolean OneWireStat = sensor.NewRead(); //Initialize OneWire sensor on location
    #if defined(WHITE_RABBIT_OBJECT)
      Serial1.print("OWs = ");
      Serial1.println(OneWireStat);
    #endif
    // sensors.getAddress(Therm, 0);
    // sensors.setResolution(Therm, 9);
    // sensors.requestTemperatures();

    TempData[i] = sensor.celsius();
...
void TCS_AutoRange()
{
  ...
  if(TCS_TestOverflow()) { //Within default range, no need for auto-range
    ...
    while(!InRange && (millis() - LocalTime) < 10000) {
      ...
      #if defined(WHITE_RABBIT_OBJECT)
        Serial1.print("Gain = ");
        Serial1.print(TCS_GainIndex);
        Serial1.print(" Int = ");
        Serial1.println(TCS_IntIndex);
      #endif
    }
  }
...
boolean TCS_TestOverflow() //FIX! Improve efficiency!
{
  ...
  #if defined(WHITE_RABBIT_OBJECT)
    Serial1.print("Overflow val = "); Serial1.println(OverflowCount);
  #endif
